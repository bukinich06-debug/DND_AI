...
| `advance_time` | `advanceTimeTool.ts` | Продвинуть время (ролевой отдых) |
| `short_rest` | `shortRestTool.ts` | Короткий отдых (механика) |
| `long_rest` | `longRestTool.ts` | Длинный отдых (механика) |
...
Продвинуть активное путешествие на N дней (по умолчанию 1). При завершении последнего отрезка — прибытие. **Время кампании продвигается на N дней** (dayIndex += N).
...
Продвинуть время кампании на заданное число слотов для **ролевого отдыха или ожидания** (сидим в таверне, чиллим, ждём). **НЕ восстанавливает HP** и **НЕ тратит кости хитов**. Не использовать для механического короткого или длинного отдыха по правилам D&D (для этого `short_rest` / `long_rest`).

Слоты суток по порядку: `morning` → `noon` → `afternoon` → `evening` → `lateEvening` → `midnight` → `night` → (следующий день) `morning`.

**Args**

| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `slots` | integer | да | Число слотов времени (минимум 1) |
...
**Короткий отдых** (`short_rest`):
- Тратит указанное число `hitDice` (кости хитов) для восстановления HP.
- **Продвигает время кампании на +1 слот** (например: утро → полдень → послеполудня и т.д.).
- **Лимит: максимум 2 коротких отдыха за игровой день** (при попытке третьего вернёт ошибку).
- Счётчик коротких отдыхов сбрасывается при длинном отдыхе или смене дня.

**Длинный отдых** (`long_rest`):
- Полное восстановление HP, половина уровня костей хитов восстанавливается, снимает 1 уровень истощения.
- **Продвигает время кампании к утру следующего дня** (`dayIndex + 1`, `timeOfDay = morning`).
- Сбрасывает счётчик коротких отдыхов на новый день.

**Ролевой отдых vs механический:**
