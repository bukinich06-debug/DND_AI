...
export const searchPlayerItemsTool: ILlmTool = {
  ...
  description:
    'Ищет предметы в инвентаре игрока. Без query — весь инвентарь. С query — кандидаты по имени/описанию; для инструментов PHB (thieves tools, воровские инструменты и т.п.) матч по стандартным именам. Для уникальных/магических предметов («лампа всевидения») бери кандидата и читай description/properties, чтобы описать эффект. Если список пуст — предмета нет. Перед использованием предмета вызывай этот tool.',
  parameters: {
    type: 'object',
    properties: {
      playerId: {
